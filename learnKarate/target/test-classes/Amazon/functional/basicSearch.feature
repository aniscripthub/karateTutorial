Feature: Basic Search on Amazon
  ...
  @getTheTitleOfProduct
  Scenario: Get The Title of Product Loaded
    * driver 'https://amazon.in'
    * driver.maximize()
    * waitFor(OR.homeAmazon.input_searchHome).click()
    * input(OR.homeAmazon.input_searchHome,"Bags")
    * waitFor(OR.homeAmazon.btn_searchSubmit).click()
    * waitForEnabled(OR.homeAmazon.div_firstResult).click()
    * waitFor(OR.productPage.text_productTitle)
    * def productTitle = text(OR.productPage.text_productTitle)
    * print productTitle
    * match productTitle contains "Backpack"